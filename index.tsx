...
import { createOpenAppsMap, GeminiOpenAppsMap, DosInstances } from './gemini95Framework';
...
const dosInstances: DosInstances = {};
...
const startMenu = document.getElementById('start-menu') as HTMLDivElement | null;
...
// --- Matrix Clippy DOM References ---
const clippyContainer = document.getElementById('matrix-clippy-container') as HTMLDivElement;
const matrixCanvas = document.getElementById('matrix-rain-canvas') as HTMLCanvasElement;
const clippyAvatarImg = document.getElementById('clippy-avatar-img') as HTMLImageElement;
const clippySpeechBubble = document.getElementById('clippy-speech-bubble') as HTMLDivElement;
const clippySpeechText = document.getElementById('clippy-speech-text') as HTMLParagraphElement;
const clippyContextMenu = document.getElementById('clippy-context-menu') as HTMLDivElement;
const toggleClippyIcon = document.getElementById('toggle-clippy-icon') as HTMLDivElement | null;
let toggleClippyIconImg: HTMLImageElement | null = null;
if (toggleClippyIcon) {
    toggleClippyIconImg = toggleClippyIcon.querySelector('img') as HTMLImageElement | null;
}
...
// --- Matrix Clippy State ---
let matrixRainIntervalId: number | null = null;
let clippySpeechTimeoutId: number | null = null;
let isClippyDragging = false;
let clippyDragOffsetX = 0;
let clippyDragOffsetY = 0;
...
async function openApp(appName: string): Promise<void> {
    if (appName === 'toggleClippy') {
        toggleClippyVisibility();
        return;
    }
...
async function initGeminiChat(windowElement: HTMLDivElement): Promise<void> {
    ...
    const sendMessage = async () => {
        ...
        try {
             // @ts-ignore
            const chat = geminiInstance.chats.create({ model: 'gemini-2.0-flash-lite', history: [] });
             // @ts-ignore
    ...
    storyButton.addEventListener('click', async () => {
        ...
        try {
            ...
            const prompt = "Write me a short creative story (250-300 words) with an unexpected twist ending. Make it engaging and suitable for all ages.";
             // @ts-ignore
            const result = await geminiInstance.models.generateContentStream({ model: 'gemini-2.0-flash-lite', contents: prompt });
            textarea.value = currentText + "\n\n";
            for await (const chunk of result) {
...
icons.forEach(icon => {
    icon.addEventListener('click', () => {
        ...
        if (appName) {
            openApp(appName); // openApp will handle toggleClippy
...
document.querySelectorAll('.start-menu-item').forEach(item => {
    item.addEventListener('click', () => {
        ...
        if (action === 'toggleClippyFromMenu') toggleClippyVisibility();
        if (startMenu) startMenu.classList.remove('active');
    });
});
...
document.addEventListener('click', (e) => {
    if (startMenu && startMenu.classList.contains('active') && !startMenu.contains(e.target as Node) && !startButton.contains(e.target as Node)) {
        startMenu.classList.remove('active');
    }
    // Hide Clippy context menu if clicked outside
    if (clippyContextMenu && clippyContextMenu.style.display === 'flex' && !clippyContextMenu.contains(e.target as Node) && !clippyContainer.contains(e.target as Node)) {
        clippyContextMenu.style.display = 'none';
    }
});
    ...
    async function getAiHint() {
        ...
        try {
            ...
            const prompt = `Minesweeper state:\n${boardState}\nShort, witty hint (1-2 sentences) for a safe move or dangerous area. Don't reveal exact mines unless certain. Hint:`;
             // @ts-ignore
            const result = await geminiInstance.models.generateContent({ model: "gemini-2.0-flash-lite", contents: [{role:"user", parts:[{text:prompt}]}], config: {temperature: 0.7}});
...
// --- Matrix Clippy Functions ---
function initMatrixClippy() {
    if (!clippyContainer || !matrixCanvas || !clippyAvatarImg || !clippySpeechBubble || !clippySpeechText || !clippyContextMenu) {
        console.error("Matrix Clippy DOM elements not found!");
        return;
    }

    // Set initial position
    clippyContainer.style.right = '30px';
    clippyContainer.style.bottom = '40px'; // Above taskbar
    clippyContainer.style.display = isClippyVisible ? 'flex' : 'none';


    const ctx = matrixCanvas.getContext('2d');
    if (!ctx) {
        console.error("Failed to get canvas context for Matrix rain.");
        return;
    }

    const W = matrixCanvas.width = clippyContainer.offsetWidth;
    const H = matrixCanvas.height = clippyContainer.offsetHeight;
    const FONT_SIZE = 12;
    const columns = Math.floor(W / FONT_SIZE);
    const drops: number[] = [];
    for (let i = 0; i < columns; i++) {
        drops[i] = Math.random() * H; // Random starting y-position
    }

    // Characters for Matrix rain - Katakana, Latin, Numerals
    ...
    if (matrixRainIntervalId) clearInterval(matrixRainIntervalId);
    matrixRainIntervalId = window.setInterval(drawMatrix, 50);


    // Draggable Clippy
    clippyContainer.addEventListener('mousedown', (e: MouseEvent) => {
        // Prevent dragging if clicking on speech bubble or avatar elements that might have their own interactions
        if ((e.target !== clippyContainer && e.target !== clippyAvatarImg && e.target !== matrixCanvas)) return;

        isClippyDragging = true;
        clippyContainer.style.cursor = 'grabbing';
        // Calculate offset from the clippyContainer's top-left corner, not page
        const rect = clippyContainer.getBoundingClientRect();
        clippyDragOffsetX = e.clientX - rect.left;
        clippyDragOffsetY = e.clientY - rect.top;

        document.addEventListener('mousemove', onClippyDrag);
        document.addEventListener('mouseup', onClippyDragEnd, { once: true });
         e.preventDefault(); // Prevent text selection or other default drag behaviors
    });

    // Context Menu
    clippyContainer.addEventListener('contextmenu', (e: MouseEvent) => {
        e.preventDefault();
        clippyContextMenu.style.left = `${e.clientX}px`;
        clippyContextMenu.style.top = `${e.clientY}px`;
        clippyContextMenu.style.display = 'flex';
    });

    // Handle context menu actions
    clippyContextMenu.querySelectorAll('.custom-context-menu-item').forEach(item => {
        item.addEventListener('click', () => {
            const action = (item as HTMLElement).dataset.action;
            handleClippyContextMenuAction(action);
            clippyContextMenu.style.display = 'none';
        });
    });

    // Initial greeting
    showClippySpeech("Matrix Clippy online. How can I assist you in this reality?", 5000);
}

function onClippyDrag(e: MouseEvent) {
    if (!isClippyDragging) return;
    let newX = e.clientX - clippyDragOffsetX;
    let newY = e.clientY - clippyDragOffsetY;

    // Constrain within viewport (consider taskbar height)
    const taskbarHeight = document.getElementById('taskbar')?.offsetHeight || 36;
    const maxX = window.innerWidth - clippyContainer.offsetWidth;
    const maxY = window.innerHeight - clippyContainer.offsetHeight - taskbarHeight;
    ...
    clippyContainer.style.top = `${newY}px`;
     // Override fixed positioning if needed during drag by setting right/bottom to auto
    clippyContainer.style.right = 'auto';
    clippyContainer.style.bottom = 'auto';
}

function onClippyDragEnd() {
    isClippyDragging = false;
    clippyContainer.style.cursor = 'grab';
    document.removeEventListener('mousemove', onClippyDrag);
     // After dragging, we might want to re-anchor to right/bottom if that's the preferred behavior
    // For now, it will stay where it's dragged. If you want it to snap back or similar, add logic here.
}


function showClippySpeech(text: string, durationMs?: number) {
    if (!clippySpeechBubble || !clippySpeechText || !isClippyVisible) return;

    clippySpeechText.textContent = text;
    clippySpeechBubble.style.display = 'block';

    if (clippySpeechTimeoutId) clearTimeout(clippySpeechTimeoutId);
    if (durationMs) {
        clippySpeechTimeoutId = window.setTimeout(() => {
            clippySpeechBubble.style.display = 'none';
        }, durationMs);
    }
}

function hideClippySpeech() {
    if (clippySpeechBubble) clippySpeechBubble.style.display = 'none';
    if (clippySpeechTimeoutId) clearTimeout(clippySpeechTimeoutId);
}

function handleClippyContextMenuAction(action?: string) {
    switch (action) {
        case 'aiChat':
            openApp('gemini');
            showClippySpeech("Opening Gemini App for your chat session...", 3000);
            break;
        case 'pingPong':
            showClippySpeech("Ping-Pong mode initiated. Use Gemini App for commands.", 5000);
            openApp('gemini');
            break;
        case 'modelInfo':
            const modelInfoText = `Matrix AI Model Information:
            • System: Matrix AI Interface
            • Features: Chat, Dev Support, Problem Solving
            • Model: Matrix-LLM-Turbo v1.5.0
            • Status: Fully Operational`;
            showClippySpeech(modelInfoText, 10000);
            break;
        case 'exitClippy':
            toggleClippyVisibility(false);
            break;
    }
}

function toggleClippyVisibility(forceShow?: boolean) {
    if (forceShow !== undefined) {
        isClippyVisible = forceShow;
    } else {
    ...
    if (clippyContainer) {
        clippyContainer.style.display = isClippyVisible ? 'flex' : 'none';
        if (!isClippyVisible) {
            hideClippySpeech(); // Hide speech bubble when Clippy is hidden
             if (matrixRainIntervalId) {
                clearInterval(matrixRainIntervalId);
                matrixRainIntervalId = null;
            }
        } else {
            // Re-initialize rain if it was stopped and Clippy is shown again
            if (!matrixRainIntervalId) initMatrixClippy(); // This will also restart the rain
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    initMatrixClippy(); // Initialize Clippy and its Matrix rain

    if (toggleClippyIcon) { // Event listener for the desktop icon
        if (toggleClippyIconImg) { // Check if img element was found
            const clippyIconSrc = "https://storage.googleapis.com/gemini-95-icons/clippy-matrix.png";
            // Force re-assign to potentially fix loading issues.
            toggleClippyIconImg.src = ''; // Clear it first
            toggleClippyIconImg.src = clippyIconSrc; // Then re-assign
        }

        toggleClippyIcon.addEventListener('click', () => {
            toggleClippyVisibility();
            if (startMenu) { // Guarding startMenu access
